import { POST_LIST_FAILURE, POST_LIST_REQUEST, POST_LIST_SUCCESS } from "../constants/postListConstants";
import { QUESTION_VOTE } from "../constants/voteConstants";

export const postListReducer = (state = { postList: [], loading: false, errorMessage: null }, action) => {
    switch (action.type) {
        case POST_LIST_REQUEST:
            return {
                ...state,
                loading: true,
                errorMessage: null
            }
        case POST_LIST_SUCCESS:
            if (action.payload.page === 1) {
                return {
                    postList: action.payload.data,
                    loading: false,
                    errorMessage: null
                }
            }
            let temp = state.postList;
            temp = temp.concat(action.payload.data);
            return {
                postList: temp,
                loading: false,
                errorMessage: null
            }
        case POST_LIST_FAILURE:
            return {
                ...state,
                loading: false,
                errorMessage: action.payload
            } 
        case QUESTION_VOTE:
            const { id, value } = action.payload
            console.log(id, value);
            let postList = state.postList;
            postList.forEach(post => {
                if (post.id === id) {
                    if (post.user_vote === null) {
                        if (value === 1) {
                            post.up_votes += 1
                        } else if (value === -1) {
                            post.down_votes += 1 
                        }
                    } else {
                        if (value === 1 && post.user_vote !== value) {
                            post.up_votes += 1
                            post.down_votes -= 1 
                        } else if (value === -1 && post.user_vote !== value) {
                            post.down_votes += 1 
                            post.up_votes -= 1
                        }
                    }
                    
                    post.rating = post.up_votes - post.down_votes;
                    post.user_vote = value;
                }
            });
            
            return {
                ...state,
                postList: postList
            }
        default:
            return state;
    }
}